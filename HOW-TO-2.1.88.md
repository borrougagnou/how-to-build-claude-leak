# How to build Claude Code CLI 2.1.88

---

The process is similar to the 1.0.34, but there's few change + There's "MORE" below the "Thank you" if you're a Pro account user

---

People talk a lot about the features that Claude-code has.

But no one explains how to build it so that Claude works on your computer. (Thank you below)

Step-by-step guide for building the Claude Code CLI
...
- Bun >= 1.3
- Git
- ripgrep (on debian/ubuntu : sudo apt install -y ripgrep)
...
## 2. Clone or unzip the src from antopic leak

Create a folder that will contain only the `src` folder (and mayyyybe `vendor`)

The `src/` (~1,900 files, ~512K lines of TypeScript) and `README.md`.<br>
There is no `package.json` or `tsconfig.json`, they must be created manually.

Then do manipulation below from this folder **(NOT FROM `src/` FOLDER !)**
...
### 5.2 Missing Source Files (in src/)
...
### 5.3 Auto-generate Remaining types.ts Files
...
This installs <del>~510</del> (as of today 530) packages.

### 6.1 Native/Internal npm Modules (in node_modules)
...
type MessageWithUuid = {
  uuid?: string
}

type MessageWithSnipMetadata = MessageWithUuid & {
  type?: string
  snipMetadata?: {
    removedUuids?: string[]
  }
...
function getRemovedUuids(messages: readonly MessageWithSnipMetadata[]): Set<string> {
...
export function isSnipBoundaryMessage(
  message: unknown,
): message is MessageWithSnipMetadata {
  return Boolean(
    message &&
      typeof message === 'object' &&
      (message as MessageWithSnipMetadata).type === 'system' &&
      Array.isArray((message as MessageWithSnipMetadata).snipMetadata?.removedUuids),
  )
}

export function projectSnippedView<T extends MessageWithSnipMetadata>(
  messages: readonly T[],
): T[] {
  const removedUuids = getRemovedUuids(messages)
...
type SnipBoundaryMessage = MessageWithUuid & {
  type?: string
  snipMetadata?: {
    removedUuids?: string[]
  }
...
export const SNIP_NUDGE_TEXT =
  'Some older context can be trimmed when needed to keep the working set focused.'
...
export function snipCompactIfNeeded<T extends SnipBoundaryMessage>(
  messages: readonly T[],
  ...
  return {
    // Preserve previously-recorded snip removals on replay without trying
    // to invent new snip behavior in checkouts that do not ship the feature.
    messages: projectSnippedView(messages),
    tokensFreed: 0,
    boundaryMessage: undefined,
  }
}
...
```bash
cat > src/components/messages/SnipBoundaryMessage.tsx << 'EOF'
...
export function SnipBoundaryMessage({ message }: Props): React.ReactNode {
  ...
  return (
    <Box marginY={1}>
      <Text dimColor>
        {removedCount > 0
          ? `Context trimmed (${removedCount} earlier message${removedCount === 1 ? '' : 's'} hidden)`
          : 'Context trimmed'}
      </Text>
    </Box>
  )
...
bun build src/entrypoints/cli.tsx \
  --target=bun \
  --outdir=dist \
  --define 'MACRO.VERSION="2.1.119"' \
  --define 'MACRO.BUILD_TIMESTAMP="2026-04-24"' \
  --define 'MACRO.BUILD_TIME="2026-04-24T12:00:00Z"' \
...
To create a binary, use Bun’s `--compile` mode.<br/>
For a Linux binary, switch from `--target=bun` to `--compile` plus a Linux target, and use `--outfile` for the binary destination.


Here's some example from the documentation:
- `bun-linux-x64-baseline`: safer for older CPUs
- `bun-linux-x64-modern`: faster, newer CPUs
- `bun-linux-x64-musl`: better for Alpine/musl systems
...
- @alesha-pro who for putting together the build process AND the source code.
- me, for improving the code, because 30 days later it wasn't working anymore....
---
...
### How to use the "Max" plan with a "Pro" acccount ?
...
You need to execute 2 script:
- `enable-auto-mode-build.sh` this one will enable the "TRANSCRIPT_CLASSIFIER" feature needed to use the "auto" mode or other thing.
- `enable-pro-max-parity.sh` this one will fix the code and implement the "max" feature as "pro" user.

With that you'll have access to:
- the plan parallelism
- the `auto` mode, now claude code will not ask your confirmation and will operate completely autonomously
  - (you can enable that with: `claude --permission-mode auto` or with ` Shift ` + ` Tab ` on the interface)
- Opus-first / Opus 1M local behavior
- related guest-pass / concurrency UI gating


### Build

Create a directory containing to store these 2 files + the `src` folder.

Execute the 2 scripts.

Now you can build with `bun run build:cli:auto`.<br />
This command to generate a binary (if you have enabled the "package.json" modification).

If you don't have enabled the "package.json" modification:
